...
cadena sub_cadena(int *padre)
{
    ...
    new_chain.tam = abs(dice_roll - dice_roll2);
    new_chain.elements = (int *)malloc(sizeof(int) * new_chain.tam);
    int min = min_between_two_numbers(dice_roll, dice_roll2);
    for (int i = 0; i < new_chain.tam; i++)
    {
        new_chain.elements[i] = padre[min];
        min++;
    }
